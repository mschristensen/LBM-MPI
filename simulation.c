...
void propagate(const param_t params, speed_t* cells, speed_t* tmp_cells)
{
        ...
        for (jj = 0; jj < params.loc_nx; jj++)
        {
            ...
            ** scratch space grid */
            if(ii == 0)
            {
              ...
              params.sendbuf_d[x_w * 3 + 2] = cells[ii*params.loc_nx + jj].speeds[7]; // south-west
              ...
              params.sendbuf_u[x_e * 3 + 0] = cells[ii*params.loc_nx + jj].speeds[5]; // north-east
              params.sendbuf_u[ jj * 3 + 1] = cells[ii*params.loc_nx + jj].speeds[2]; // north
              params.sendbuf_u[x_w * 3 + 2] = cells[ii*params.loc_nx + jj].speeds[6]; // north-west
            } else {
              tmp_cells[ii *params.loc_nx + jj].speeds[0] = cells[ii*params.loc_nx + jj].speeds[0]; // central cell
              tmp_cells[ii *params.loc_nx + x_e   ].speeds[1] = cells[ii*params.loc_nx + jj].speeds[1]; // east
              tmp_cells[y_n*params.loc_nx + jj].speeds[2] = cells[ii*params.loc_nx + jj].speeds[2]; // north
    ...
    for(jj = 0; jj < params.loc_nx; jj++)
    {
      tmp_cells[(params.loc_ny - 1) * params.loc_nx + jj].speeds[8] = params.recvbuf[jj * 3 + 0];
      tmp_cells[(params.loc_ny - 1) * params.loc_nx + jj].speeds[4] = params.recvbuf[jj * 3 + 1];
      tmp_cells[(params.loc_ny - 1) * params.loc_nx + jj].speeds[7] = params.recvbuf[jj * 3 + 2];
    }
    ...
    for(jj = 0; jj < params.loc_nx; jj++)
    {
      tmp_cells[jj].speeds[5] = params.recvbuf[jj * 3 + 0];
      tmp_cells[jj].speeds[2] = params.recvbuf[jj * 3 + 1];
      tmp_cells[jj].speeds[6] = params.recvbuf[jj * 3 + 2];
    }
}
...
void rebound(const param_t params, speed_t* cells, speed_t* tmp_cells, int* obstacles)
{
    ...
    for (ii = 0; ii < params.loc_ny; ii++)
    {
        ...
        {
            /* if the cell contains an obstacle */
            if (obstacles[(ii + params.rank * params.loc_ny)*params.nx + jj])//ii*params.nx + (obs_jj + (params.rank * params.loc_nx))])
            {
                /* called after propagate, so taking values from scratch space
                ** mirroring, and writing into main grid */
                cells[ii*params.loc_nx + (jj)].speeds[1] = tmp_cells[ii*params.loc_nx + (jj)].speeds[3];
